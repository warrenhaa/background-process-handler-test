...
let { manage } = require('../handler/CloudBridgeHandler')
...
const rateLimiter = new RateLimiterRedis({
    ...
    points:  parseInt(process.env.SQS_CLOUD_BRIDGE_QUEUE_RATELIMIT) ,
    duration: parseInt(process.env.SQS_CLOUD_BRIDGE_QUEUE_RATELIMIT_DURATION) 
});

let pointer = 0
const app = Consumer.create({
    queueUrl: process.env.SQS_CLOUD_BRIDGE_QUEUE_URL,
    batchSize: parseInt(process.env.SQS_CLOUD_BRIDGE_QUEUE_BATCHSIZE),
    handleMessage: async (message) => {
        if (paused) {
            throw new Error('Rate limit exceeded');
        }
        try {
            await rateLimiter.consume(`cloud-bridge-sqs-${instanceId}`);
        } catch (rejRes) {
            ...
            if (!paused) {
                ...
                const msRemaining = rejRes.msBeforeNext || (parseInt(process.env.SQS_CLOUD_BRIDGE_QUEUE_RATELIMIT_DURATION)*1000);
                app.stop();
                setTimeout(() => {
                    paused = false;
                    setImmediate(() => {
        ...
        // console.log("🚀 ~ file: CloudBridgeQueueConsumer.js:18 ~ handleMessage: ~ obj:", obj)
        manage(obj).then(result => {
            ...
            if (err && err.message) {
                ...
                const issueExists = await getIssue(params).catch(err => {
                    console.log("🚀 ~ file: CloudBridgeQueueConsumer.js:30 ~ issueExists ~ err:", err)
                })
                if (issueExists == false) {
                    await createIssue(params).catch(err => {
                        console.log("🚀 ~ file: CloudBridgeQueueConsumer.js:34 ~ awaitcreateIssue ~ err:", err)
                    })
                }
