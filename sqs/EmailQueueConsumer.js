...
var { manage } = require('../handler/EmailHandler')
...
const rateLimiter = new RateLimiterRedis({
    ...
    points:  parseInt(process.env.EMAIL_QUEUE_RATELIMIT) ,
    duration: parseInt(process.env.EMAIL_QUEUE_RATELIMIT_DURATION) 
});

let pointer = 0
const app = Consumer.create({
    queueUrl: process.env.EMAIL_QUEUE_URL,
    batchSize: parseInt(process.env.EMAIL_QUEUE_BATCHSIZE),
    handleMessage: async (message) => {
        ...
        try {
            await rateLimiter.consume(`email-sqs-${instanceId}`);
        } catch (rejRes) {
            ...
            if (!paused) {
                ...
                const msRemaining = rejRes.msBeforeNext || (parseInt(process.env.EMAIL_QUEUE_RATELIMIT_DURATION)*1000);
                app.stop();
                setTimeout(() => {
                    paused = false;
                    setImmediate(() => {
        ...
        // Logger.info("EmailQueueMessage-Received", { "pointers": pointer })
        manage(obj, pointer).then(result => {
            ...
            if (err && err.message) {
                ...
                const issueExists = await getIssue(params).catch(err => {
                    console.log("Error- EmailQueueConsumer CreateIssue Error", err);
                })
