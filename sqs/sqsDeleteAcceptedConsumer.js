...
var { manage } = require('../handler/DeviceDeleteHandler')
...
const rateLimiter = new RateLimiterRedis({
    ...
    points:  parseInt(process.env.SQS_DEVICE_DELETE_RATELIMIT) ,
    duration: parseInt(process.env.SQS_DEVICE_DELETE_RATELIMIT_DURATION) 
});

let pointer = 0
const app = Consumer.create({
    queueUrl: process.env.SQS_DEVICE_DELETE_URL,
    batchSize: parseInt(process.env.SQS_DEVICE_DELETE_BATCHSIZE),
    handleMessage: async (message) => {
        if (paused) {
            throw new Error('Rate limit exceeded');
        }
        try {
            await rateLimiter.consume(`delete-accepted-sqs-${instanceId}`);
        } catch (rejRes) {
            ...
            if (!paused) {
                ...
                const msRemaining = rejRes.msBeforeNext || (parseInt(process.env.SQS_DEVICE_DELETE_RATELIMIT_DURATION)*1000);
                app.stop();
                setTimeout(() => {
                    paused = false;
                    setImmediate(() => {
        ...
        // Logger.info("Delete-Device-Message-Received", { "pointer": pointer })
