...
const { manage } = require('../handler/SMSHandler')
...
const rateLimiter = new RateLimiterRedis({
    ...
    points:  parseInt(process.env.SMS_QUEUE_RATELIMIT), 
    duration: parseInt(process.env.SMS_QUEUE_RATELIMIT_DURATION) 
});

let pointer = 0
const app = Consumer.create({
    queueUrl: process.env.SMS_QUEUE_URL,
    batchSize: parseInt(process.env.SMS_QUEUE_BATCHSIZE),
    handleMessage: async (message) => {
        ...
        try {
            await rateLimiter.consume(`sms-sqs-${instanceId}`);
        } catch (rejRes) {
            ...
            if (!paused) {
                ...
                const msRemaining = rejRes.msBeforeNext || (parseInt(process.env.SMS_QUEUE_RATELIMIT_DURATION)*1000);
                app.stop();
                setTimeout(() => {
                    paused = false;
                    setImmediate(() => {
