...
const { manage } = require('../handler/SunsetSunriseHandler')
const { createIssue } = require('../services/GitlabTicketService')
...
const rateLimiter = new RateLimiterRedis({
    ...
    points:  parseInt(process.env.SUNSET_SUNRISE_QUEUE_RATELIMIT),
    duration: parseInt(process.env.SUNSET_SUNRISE_QUEUE_RATELIMIT_DURATION) 
});

let pointer = 0
const app = Consumer.create({
    queueUrl: process.env.SUNSET_SUNRISE_QUEUE_URL,
    batchSize: parseInt(process.env.SUNSET_SUNRISE_QUEUE_BATCHSIZE),
    handleMessage: async (message) => {
        if (paused) {
            throw new Error('Rate limit exceeded');
        }
        try {
            await rateLimiter.consume(`sunset-sunrise-sqs-${instanceId}`);
        } catch (rejRes) {
            //console.warn('Rate limit exceeded, delaying message processing...');
            ...
            if (!paused) {
                ...
                const msRemaining = rejRes.msBeforeNext || (parseInt(process.env.SUNSET_SUNRISE_QUEUE_RATELIMIT_DURATION)*1000);
                app.stop();
                setTimeout(() => {
                    paused = false;
                    setImmediate(() => {
        ...
        //Logger.info("SMS-Queue-Message-Received", { "pointers": pointer })
        manage().then(result => {
            ...
            if (err && err.message) {
                ...
                await createIssue(params).catch(err => {
                    console.log("Error- SMSQueueConsumer CreateIssue Error", err);
                });
            }
        })
    }
