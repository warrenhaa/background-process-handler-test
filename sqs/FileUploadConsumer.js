...
var { ReUploadJsonUrl } = require('../handler/DeviceUpdateHandler')
...
const rateLimiter = new RateLimiterRedis({
    ...
    points:  parseInt(process.env.FILE_UPLOAD_QUEUE_RATELIMIT) ,
    duration: parseInt(process.env.FILE_UPLOAD_QUEUE_RATELIMIT_DURATION)  
});

var pointer = 0
const app = Consumer.create({
    queueUrl: process.env.FILE_UPLOAD_QUEUE_URL,
    batchSize: parseInt(process.env.FILE_UPLOAD_QUEUE_BATCHSIZE),
    handleMessage: async (message) => {
        if (paused) {
            throw new Error('Rate limit exceeded');
        }
        try {
            await rateLimiter.consume(`file-upload-sqs-${instanceId}`);
        } catch (rejRes) {
            ...
            if (!paused) {
                ...
                const msRemaining = rejRes.msBeforeNext || (parseInt(process.env.FILE_UPLOAD_QUEUE_RATELIMIT_DURATION)*1000);
                app.stop();
                setTimeout(() => {
                    paused = false;
                    setImmediate(() => {
        ...
        ReUploadJsonUrl(obj).then(result => {
        }).catch(async err => {
            if (err && err.message) {
                ...
                const issueExists = await getIssue(params).catch(err => {
                    console.log("Error- FileUploadConsumer CreateIssue Error", err);
                })
