...
var { manage } = require('../handler/ActivityLogHandler')
...
const rateLimiter = new RateLimiterRedis({
    ...
    points: parseInt(process.env.ACTIVITY_LOG_QUEUE_RATELIMIT), 
    duration: parseInt(process.env.ACTIVITY_LOG_QUEUE_RATELIMIT_DURATION) 
});

const app = Consumer.create({
    queueUrl: process.env.ACTIVITY_LOG_QUEUE_URL,
    batchSize: parseInt(process.env.ACTIVITY_LOG_QUEUE_BATCHSIZE),
    handleMessage: async (message) => {
        console.log("🚀 ~ activity logs message:", message)
        if (paused) {
            throw new Error('Rate limit exceeded');
        }
        try {
            await rateLimiter.consume(`activity-logs-sqs-${instanceId}`);
        } catch (rejRes) {
            if (!paused) {
                paused = true;
                const now = Date.now();
                ...
                const msRemaining = rejRes.msBeforeNext || (parseInt(process.env.ACTIVITY_LOG_QUEUE_RATELIMIT_DURATION)*1000);
                app.stop();
                setTimeout(() => {
                    paused = false;
                    setImmediate(() => {
        ...
        manage(obj).then(result => {
            ...
            if (err && err.message) {
                ...
                const issueExists = await getIssue(params).catch(err => {
                    ...
                    console.log("Error- ActivityLogQueueConsumer CreateIssue Error", err)
