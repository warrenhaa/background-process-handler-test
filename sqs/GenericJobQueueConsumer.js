...
var { manage } = require('../handler/GenericJobHandler')
...
const rateLimiter = new RateLimiterRedis({
    ...
    points:  parseInt(process.env.SQS_GENERIC_JOB_QUEUE_RATELIMIT) ,
    duration: parseInt(process.env.SQS_GENERIC_JOB_QUEUE_RATELIMIT_DURATION)  
});

let pointer = 0
const app = Consumer.create({
    queueUrl: process.env.SQS_GENERIC_JOB_QUEUE_URL,
    batchSize:  parseInt(process.env.SQS_GENERIC_JOB_QUEUE_BATCHSIZE),
    handleMessage: async (message) => {
        if (paused) {
            throw new Error('Rate limit exceeded');
        }
        try {
            await rateLimiter.consume(`generic-job-sqs-${instanceId}`);
        } catch (rejRes) {
            ...
            if (!paused) {
                ...
                const msRemaining = rejRes.msBeforeNext || (parseInt(process.env.SQS_GENERIC_JOB_QUEUE_RATELIMIT_DURATION)*1000);
                app.stop();
                setTimeout(() => {
                    paused = false;
                    setImmediate(() => {
        ...
        // Logger.info("Generic-Job-Queue-Message-Received", { "pointer": pointer, obj })
        manage(obj).then(result => {
            //Logger.info("Generic-Job-Queue-Message-Success", { "pointer": pointer })
        }).catch(async err => {
            if (err && err.message) {
                ...
                const issueExists = await getIssue(params).catch(err => {
                    console.log("Error- GenericJobqueueConsumer CreateIssue Error", err);
                })
