...
const rateLimiter = new RateLimiterRedis({
    ...
    points:  parseInt(process.env.SQS_RULE_HANDLER_RATELIMIT),
    duration: parseInt(process.env.SQS_RULE_HANDLER_RATELIMIT_DURATION) 
});


let pointer = 0
const app = Consumer.create({
    queueUrl: process.env.SQS_RULE_HANDLER_URL,
    batchSize: parseInt(process.env.SQS_RULE_HANDLER_BATCHSIZE),
    handleMessage: async (message) => {
        if (paused) {
            throw new Error('Rate limit exceeded');
        }
        try {
            await rateLimiter.consume('sqs-message');
        } catch (rejRes) {
            ...
            if (!paused) {
                ...
                const msRemaining = rejRes.msBeforeNext || (parseInt(process.env.SQS_RULE_HANDLER_RATELIMIT_DURATION)*1000);
                app.stop();
                setTimeout(() => {
                    paused = false;
                    setImmediate(() => {
        ...
        // console.log(obj)
        // Logger.info("Rule-Handler-Message-Received", { "pointers": pointer })
        ...
        //             console.log("Error- sqsRuleHandlerConsumer CreateIssue Error", err);
        //         });
        //     }
        // })
